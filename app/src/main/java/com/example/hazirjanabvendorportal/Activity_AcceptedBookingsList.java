...
public class Activity_AcceptedBookingsList extends AppCompatActivity implements RecyclerViewInterface{

    RecyclerView rvAcceptedRequestsList;
    public static List<Class_ServiceBooking> acceptedRequestsList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_accepted_bookings_list);

        toolbar = Toolbar.getInstance();
        toolbar.setup(this);
        NotificationBadge.getInstance(this).setCounter("Accepted_services", 0);

        rvAcceptedRequestsList = findViewById(R.id.RequestsRecyclerView);
        rvAcceptedRequestsList.setLayoutManager(new LinearLayoutManager(this));

        llNoRequests = findViewById(R.id.llNoRequests);

        acceptedRequestsList = new ArrayList<>();
        new Activity_AcceptedBookingsList.GetAcceptedRequests(this).execute();
        ...
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                ...
                NotificationBadge.getInstance(Activity_AcceptedBookingsList.this).setCounter("Accepted_services", 0);
                new Activity_AcceptedBookingsList.GetAcceptedRequests(Activity_AcceptedBookingsList.this).execute();
            }
        });


        //acceptedRequestsList.add(new Class_ServiceBooking(1, 1, 1, 1, "Karachi", "Gulshan",new Date(123, 1, 1), new Time(1,2,3), "Description", null, "Pending"));
        adapter_serviceBookings = new Adapter_ServiceBookings(Activity_AcceptedBookingsList.this, acceptedRequestsList, this);
        rvAcceptedRequestsList.setAdapter(adapter_serviceBookings);
...
//        finish();
    ...
    public void onItemClicked(int adapterPosition) {
        ...
        if(acceptedRequestsList.isEmpty()){
            return;
        }
        Intent intent = new Intent(this, Activity_RequestDetails.class);
        intent.putExtra("id", acceptedRequestsList.get(adapterPosition).getId());
        intent.putExtra("customerId", acceptedRequestsList.get(adapterPosition).getUser_id());
        intent.putExtra("serviceId", acceptedRequestsList.get(adapterPosition).getService_id());
        intent.putExtra("vendorId", acceptedRequestsList.get(adapterPosition).getVendor_id());
        intent.putExtra("city", acceptedRequestsList.get(adapterPosition).getCity());
        intent.putExtra("address", acceptedRequestsList.get(adapterPosition).getAddress());
        if (acceptedRequestsList.get(adapterPosition).getDate() != null) {
            Log.d("Date", acceptedRequestsList.get(adapterPosition).getDate().toString());
            intent.putExtra("date", acceptedRequestsList.get(adapterPosition).getDate());
        } else
            Log.d("Date", "null");

        if (acceptedRequestsList.get(adapterPosition).getTime() != null) {
            Log.d("Time", acceptedRequestsList.get(adapterPosition).getTime().toString());
            intent.putExtra("time", acceptedRequestsList.get(adapterPosition).getTime());
        } else{
            Log.d("Time", "null");
        }
        intent.putExtra("description", acceptedRequestsList.get(adapterPosition).getDescription());

        Blob pictureBlob = acceptedRequestsList.get(adapterPosition).getPicture();
        Bitmap bitmap = null;
...
//                Log.d("BlobData", "Bitmap: " + bitmap.toString());
        ...
        intent.putExtra("type", acceptedRequestsList.get(adapterPosition).getType());
        intent.putExtra("status", acceptedRequestsList.get(adapterPosition).getStatus());

        startActivity(intent);
    }

    public class GetAcceptedRequests extends AsyncTask<Void, Void, String> {
        private Context context;
        private int vendorId;
        private Class_SingletonVendor singletonVendor;

        public GetAcceptedRequests(Context context) {
...
//                        String date = new Date(2021, 1, 1).toString();
                        ...
                        if(bookingInfo.getStatus().equals("Accepted") || bookingInfo.getStatus().equals("In-progress")){
                            acceptedRequestsList.add(bookingInfo);
                            NotificationBadge.getInstance(Activity_AcceptedBookingsList.this).incrementCounter("Accepted_services");
                            Log.d("Check3:","Done");
                        }
                        Log.d("Check","Done");
                    }
