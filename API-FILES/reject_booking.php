...
try {
    ...
    $query = "UPDATE booking_normal SET status = 'Rejected' WHERE id = :booking_id";
    ...
    if ($stmt->rowCount() > 0) {
        echo json_encode(array("Status" => 1, "Message" => "Booking status updated to 'rejected' for booking ID: $bookingId"));
    } else {
